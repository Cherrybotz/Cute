...
_I am Stawberry Bot! 🍓 one of the Whatsapp bots that is ready to help you make things easier like making stickers and more, if you want to request a feature, please type #request message or the feature you want!_ ~https://wa.me/94726083178~`
...
return conn.send3ButtonLoc(m.chat, await conn.resize(pickRandom(global.anime), 300, 300), ori, `Note : If you are using an old whatsapp or mod and the button is not visible, type it right away ${_p}simplemenu`, 'COMMAND', '.simplemenu', 'DONATION', '.donasi', 'RENT BOT', '.sewa', m)
...
                 .replace(/%islimit/g, menu.limit ? 'Ⓛ' : '')
 ...
      } 
     })*/
await conn.send3ButtonLoc(m.chat, await conn.resize(await (await fetch('https://telegra.ph/file/7054fc1e7eccc5f2cdfe0.jpg')).buffer(), 300, 200), '◈┈┉────[ *DASHBOARD* ]────┉┈◈' , text.trim(), 'Owner', '.owner', 'Donasi', '.donasi', 'Rules', '.rules', m)
//await conn.send2ButtonLoc(m.chat, await conn.resize(pickRandom(global.hentai), 300, 200), text.trim(), text.trim(), `Aktif Selama : ${uptime}\n${week} ${date}\n${dateIslamic}`, 'Owner', `${_p}owner`, 'Donasi', `${_p}donasi`, m)
